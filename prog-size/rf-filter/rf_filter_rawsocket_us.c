  ...
  rf: {
    ...
    if (pkt_leaf != NULL) {
      ...
      if (argmax == 0 || argmax == 1) {
        bool is_anomaly = (bool)(argmax);
        pkt.is_anomaly = argmax;
        #ifdef DEBUG_BUILD
...
void rf_filter(int seconds, int64_t *childrenLeft, int64_t *childrenRight, int64_t *value, int64_t *feature, int64_t *threshold, int M, int* counter) {
  ...
  while (true) {
    if ((len=recv(sock, data, sizeof(data), 0)) <= 0) {
      ...
      int flag = filter(data, childrenLeft, childrenRight, value, feature, threshold, M);
      /* printf("%d\n", flag); */
      if (flag == -2) {
        cnt++;
      }
