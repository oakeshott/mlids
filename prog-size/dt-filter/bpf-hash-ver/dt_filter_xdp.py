            ...
            try:
                ...
                for k in dropcnt.keys():
                    ...
                    if val:
                        delta = val - prev
                        prev = val
                        print("{}: {} pkt/s".format(dt, delta))
                time.sleep(1)
                # for k, v in feats.items():
                #     print(k, v.sport, v.dport, v.protocol, v.tot_len, v.interval_time, v.direction, v.is_anomaly)
                # for k, v in feats2.items():
                #     print(k, v.sport, v.dport, v.protocol, v.tot_len, v.interval_time, v.direction)
                # for k, v in tmp.items():
                #     print(k, "cur node", v.value)
                # for k, v in tmp2.items():
                #     print(k, "anomaly", v.value)
                # for k, v in tmp3.items():
                #     print(k, "cur_val", v.value)
                    # print(v.protocol,
                    #         inet_ntop(AF_INET, pack("I", v.saddr)).encode(),
                    #         inet_ntop(AF_INET, pack("I", v.daddr)).encode(),
                    #         v.sport, v.dport,
                    #         v.tot_len,
                    #         v.last_packet_timestamp)
