...
int nn_xdp_drop_packet(struct xdp_md *ctx) {
  ...
  nn: {
    ...
    if (pkt_leaf != NULL) {
      ...
      int8_t x_q2[H1];
      int s_x2 = *((int*)layer_2_s_x.lookup_or_init(&_zero, &_zero));
      int s_x_inv2 = *((int*)layer_2_s_x_inv.lookup_or_init(&_zero, &_zero));
      int scale_factor_int2 = (s_x2 + ROUND_CONST) >> FXP_VALUE;
      int scale_factor_frac2 = s_x2 - (scale_factor_int << FXP_VALUE);
      for (m = 0; m < H1; m++) {
        accumulator = 0;
        for (k = 0; k < INPUT_DIM; k++) {
          _k = k*H1 + m;
        ...
        if (tensor_int > INT8_MAX_VALUE*s_x_inv2) {
          x_q2[m] = (int8_t)INT8_MAX_VALUE;
        } else if (tensor_int < -INT8_MAX_VALUE*s_x_inv2) {
          x_q2[m] = -(int8_t)INT8_MAX_VALUE;
        } else {
          tensor_frac = out - (tensor_int << FXP_VALUE);
          rounded_value = tensor_int*scale_factor_frac2 + scale_factor_int2*tensor_frac;
          rounded_value += (tensor_frac*scale_factor_frac2 + ROUND_CONST) >> FXP_VALUE;
          rounded_value = ((rounded_value + ROUND_CONST) >> FXP_VALUE) + tensor_int*scale_factor_int2;
          x_q2[m] = (int8_t)rounded_value; /* store quantized value in output tensor */
        }
      }
      int s_x3 = *((int*)layer_3_s_x.lookup_or_init(&_zero, &_zero));
      int s_x_inv3 = *layer_3_s_x_inv.lookup_or_init(&_zero, &_zero);
      int scale_factor_int3 = (s_x3 + ROUND_CONST) >> FXP_VALUE;
      int scale_factor_frac3 = s_x3 - (scale_factor_int << FXP_VALUE);
      int8_t x_q3[H2];
      // mat_mult(x_q2, w, output, dimension_layer);
      for (m = 0; m < H2; m++) {
        accumulator = 0;
        for (k = 0; k < H1; k++) {
          _k = k*H2 + m;
          weight = *(int8_t*)layer_2_weight.lookup_or_init(&_k, &_zero);
          accumulator += x_q2[k] * weight;
        }
        ...
        tensor_int = (out + ROUND_CONST) >> FXP_VALUE;
        if (tensor_int > INT8_MAX_VALUE*s_x_inv3) {
          x_q3[m] = (int8_t)INT8_MAX_VALUE;
        } else if (tensor_int < -INT8_MAX_VALUE*s_x_inv3) {
          x_q3[m] = -(int8_t)INT8_MAX_VALUE;
        } else {
          tensor_frac = out - (tensor_int << FXP_VALUE);
          rounded_value = tensor_int*scale_factor_frac3 + scale_factor_int3*tensor_frac;
          rounded_value += (tensor_frac*scale_factor_frac3 + ROUND_CONST) >> FXP_VALUE;
          rounded_value = ((rounded_value + ROUND_CONST) >> FXP_VALUE) + tensor_int*scale_factor_int3;
          x_q3[m] = (int8_t)rounded_value; /* store quantized value in output tensor */
        }
      }
      // mat_mult(x_q3, w, output, dimension_layer);
      int argmax_over_cols = 0;
      int threashold = -INT_MAX;
      for (m = 0; m < OUTPUT_DIM; m++) {
        accumulator = 0;
        for (k = 0; k < H2; k++) {
          _k = k*OUTPUT_DIM + m;
          weight = *(int8_t*)layer_3_weight.lookup_or_init(&_k, &_zero);
          accumulator += x_q3[k] * weight;
        }
        _m = m;
        ...
        if (out2 > threashold) {
          threashold = out2;
          argmax_over_cols = _m; // return column
        }
      }
      bpf_trace_printk("%d\\n", argmax_over_cols);
      // u32 val = 0, *vp;
...
if __name__ == '__main__':
    ...
    b = BPF(text=bpf_text, debug=0x10,  cflags=["-w"],
            # allow_rlimit=True,
