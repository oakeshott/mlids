...
BPF_PERCPU_HASH(timestamp, int64_t, int64_t);
...
int nn1(struct xdp_md *ctx) {
  ...
  _m = 0;
  out = *out_input.lookup_or_init(&_m, &_zero64);
  bpf_trace_printk("Out Tail call#2 RX queue id %d, %lld\\n", ctx->rx_queue_index, out);
  ...
  // bpf_trace_printk("Out2 Tail call#2 RX queue id %d, %lld\\n", ctx->rx_queue_index, out);
  // int64_t ts = *(int64_t*)timestamp.lookup_or_init(&_zero64, &_zero64);
  // bpf_trace_printk("Tail call#2 RX queue id %d, %lld\\n", ctx->rx_queue_index, ts);
...
int nn2(struct xdp_md *ctx) {
  ...
  // _m = 0;
  // out = *out_input2.lookup_or_init(&_m, &_zero64);
  // bpf_trace_printk("Out2 Tail call#3 RX queue id %d, %lld\\n", ctx->rx_queue_index, out);
  // mat_mult(x_q, w, output, dimension_layer);
  ...
  // int64_t ts = *(int64_t*)timestamp.lookup_or_init(&_zero64, &_zero64);
  // bpf_trace_printk("Tail call#3 RX queue id %d, %lld\\n", ctx->rx_queue_index, ts);
  u32 val = 0, *vp;
  ...
  nn: {
    ...
    if (pkt_leaf != NULL) {
      ...
      for (k = 0; k < INPUT_DIM; k++) {
        ...
        x[k] = ((x[k] - out) * _data_scale + ROUND_CONST) >> FXP_VALUE;
      }
      ...
      _m = 0;
      out = *out_input.lookup_or_init(&_m, &_zero64);
      bpf_trace_printk("Out Tail call#1 RX queue id %d, %lld\\n", ctx->rx_queue_index, out);
      // timestamp.update(&_zero64, &ts);
      // bpf_trace_printk("Tail call#1 RX queue id %d, %lld\\n", ctx->rx_queue_index, ts);
...
if __name__ == '__main__':
    ...
    b = BPF(text=bpf_text, debug=0x0,  cflags=["-w"],
            # allow_rlimit=True,
