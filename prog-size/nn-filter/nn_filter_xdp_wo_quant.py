...
BPF_ARRAY(layer_1_weight, int64_t, LAYER_1_WEIGHT);
BPF_ARRAY(layer_2_weight, int64_t, LAYER_2_WEIGHT);
BPF_ARRAY(layer_3_weight, int64_t, LAYER_3_WEIGHT);
...
int nn1(struct xdp_md *ctx) {
  ...
  int _zero = 0;
  int64_t _zero64 = 0;
  int rounded_value, tensor_int, tensor_frac, scale_factor_int, scale_factor_frac, s_w_inv, s_x, s_x_inv, out_value;
  int64_t accumulator;
  int64_t out;
  int64_t weight, x_q[H1];
  ...
  for (m = 0; m < H2; m++) {
    ...
    for (k = 0; k < H1; k++) {
      _k = k*H2 + m;
      weight = *(int64_t*)layer_2_weight.lookup_or_init(&_k, &_zero);
...
int nn2(struct xdp_md *ctx) {
  ...
  int64_t _zero64 = 0;
  int rounded_value, tensor_int, tensor_frac, scale_factor_int, scale_factor_frac, s_w_inv, s_x, s_x_inv, out_value;
  int64_t out, accumulator;
  int64_t weight, x_q[H2];
  ...
  for (m = 0; m < OUTPUT_DIM; m++) {
    ...
    for (k = 0; k < H2; k++) {
      _k = k*OUTPUT_DIM + m;
      weight = *(int64_t*)layer_3_weight.lookup_or_init(&_k, &_zero);
      accumulator += x_q[k] * weight;
    }
    _m = m;
  ...
  nn: {
    ...
    if (pkt_leaf != NULL) {
      ...
      for (m = 0; m < H1; m++) {
        ...
        for (k = 0; k < INPUT_DIM; k++) {
          _k = k*H1 + m;
          weight = *(int64_t*)layer_1_weight.lookup_or_init(&_k, &_zero);
          accumulator += x[k] * weight;
        }
...
if __name__ == '__main__':
    ...
    try:
        ...
        map_bpf_table(layer_1_weight,  params['layer_1_weight'],  'int64_t')
        map_bpf_table(layer_2_weight,  params['layer_2_weight'],  'int64_t')
        map_bpf_table(layer_3_weight,  params['layer_3_weight'],  'int64_t')
