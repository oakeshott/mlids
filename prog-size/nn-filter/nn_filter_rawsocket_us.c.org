...
#define DEBUG_BUILD
#define DEVICE "router-veth1"
#define DEVICE2 "router-veth2"
#ifdef DEBUG_BUILD
#define DEBUG printf
#endif
#define TREE_LEAF -1
...
void nn_filter(int seconds) {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    strncpy(ifreq.ifr_name, devices[i], sizeof(ifreq.ifr_name) - 1);
    if (ioctl(sock[i], SIOCGIFINDEX, &ifreq) != 0) {
      perror("ioctl");
    }
    sockaddr.sll_ifindex = ifreq.ifr_ifindex;
    sockaddr.sll_family = PF_PACKET;
    sockaddr.sll_protocol = htons(ETH_P_ALL);
    if (bind(sock[i], (struct sockaddr *)&sockaddr, sizeof(sockaddr))!=0) {
      perror("bind");
    }
  }
  u_int64_t ts1, ts2;
  ...
  start = get_nsecs();
  while (true) {
    if ((len=recv(sock[0], data, sizeof(data), 0)) <= 0) {
      perror("recv");
    } else {
